...
def interpgaus3d(xp, yp, time, zp, dx, dy, dt, 
                                radius, alpha_d, alpha_t):

    # Find all NaNs and do not select them
    no_nan = ~np.isnan(zp)
    
    # Remove data wiht NaN's
    xp, yp, zp, tp = xp[no_nan], yp[no_nan], zp[no_nan],\
                            time[no_nan]
...
if __name__ == '__main__':
    ...
    Xi, Yi, Ti, Zi, Ei, Ni = interpgaus3d(xp, yp, time=tp, zp=zp, dx=dx, 
                                            dy=dy, dt=dt, radius=radius, 
                                            alpha_d=alpha_d, alpha_t=alpha_t)
